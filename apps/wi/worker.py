...
class Worker(BaseWorker):
    ...
    async def command(self, message: discord.Message, command: str, args: list):
        ...
        if args[0] == "taifu":
            ...
            embed.add_field(name="座標", value=latitude + longitude, inline=False)
            embed.add_field(name="大きさ", value=size, inline=False)
            embed.add_field(name="勢力", value=strength, inline=False)
